...
class ConversationController(ABC):
    ...
    async def listen(self):
        while len(self.patient_track) > (self.vad_pos + VAD_FRAME_BYTES):
            frame = self.patient_track[self.vad_pos:self.vad_pos + VAD_FRAME_BYTES]
            if self.check_silence:
                ...
                if self.frames and self.check_stopwords and ((not is_speech) or len(self.frames)>30000):
    ...
    async def receive_inbound(self, received_bytes):
        ...
        self.internal_pos = len(self.patient_track)
